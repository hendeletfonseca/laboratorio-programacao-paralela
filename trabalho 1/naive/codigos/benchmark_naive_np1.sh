...
# Forca execucao com apenas 1 processo MPI.
NP="1"
LIMIT="${LIMIT:-50000000}"
RESULT_CSV="${1:-$SCRIPT_DIR/resultados_execucao_np1.csv}"
...
for src_path in "${sources[@]}"; do
    ...
    exe_name="${base_name}_np1"
    exe_path="$BIN_DIR/$exe_name"

    compile_log="$LOG_DIR/${base_name}_compile_np1.log"
    run_log="$LOG_DIR/${base_name}_run_np1.log"
